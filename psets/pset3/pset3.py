...
class MinimaxAgent(MultiAgentSearchAgent):
    def get_action(self, game_state: GameState) -> Action:
        ...
        print(actions)
        for next_letter in actions:
            if self.index == 0:
                d[next_letter] = self.min_val(game_state.generate_successor(next_letter))[0]
            else:
                d[next_letter] = self.max_val(game_state.generate_successor(next_letter))[0]
        v = float('-inf') if self.index == 0 else float('inf')
        r = ''
        print(d)
        for action in d:
            if self.index == 0:
                if (d[action] > v):
                    ...
                    if actions.index(action) < actions.index(r):
                        r = action
            else:
                if (d[action] < v):
                    v = d[action]
    ...
    def max_val(self, game_state: GameState) -> Tuple[float, Optional[Action]]:
        ...
        prev_value = value
        action = ''
        for children in game_state.get_actions():
            value = max(value, self.min_val(game_state.generate_successor(children))[0])
            if prev_value != value:
                action = children
                prev_value = value
        return (value, action)
    ...
    def min_val(self, game_state: GameState) -> Tuple[float, Optional[Action]]:
        ...
        action = ''
        for children in game_state.get_actions():
            value = min(value, self.max_val(game_state.generate_successor(children))[0])
            if prev_value != value:
                action = children
                prev_value = value
        return (value, action)
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    def get_action(self, game_state: GameState):
        _, state = self.val(game_state, float('-inf'), float('inf'))
        # print(state.prefix)
        # print(game_state.prefix)
        # print(v)
        return state.prefix[len(game_state.prefix)]
        # d = {}
        # actions = game_state.get_actions()

        # for next_letter in actions:
        #     d[next_letter] = self.val(game_state.generate_successor(next_letter), float('-inf'), float('inf'))
        # v = float('-inf') if self.index == 0 else float('inf')
        # r = ''
        # for action in d:
        #     if self.index == 0:
    ...
    def val(self, game_state: GameState, alpha, beta):
        if game_state.is_terminal():
            return (game_state.value(), game_state)
        if game_state.index == 0:
            actions = game_state.get_actions()
            best = None
            prev = alpha
            for next_letter in actions:
                child = game_state.generate_successor(next_letter)
                alpha = max(alpha, self.val(child, alpha, beta)[0])
                if prev != alpha:
                    best = child
                    prev = alpha
                if beta <= alpha:
            ...
            for next_letter in actions:
                child = game_state.generate_successor(next_letter)
                beta = min(beta, self.val(child, alpha, beta)[0])
                if prev != beta:
                    best = child
                    prev = beta
                if beta <= alpha:
        ...
        # for next_letter in actions:
        #     if self.index == 0:
        #         d[next_letter] = self.min_val(game_state.generate_successor(next_letter), float('-inf'), float('inf'))[0]
        #     else:
        #         d[next_letter] = self.max_val(game_state.generate_successor(next_letter), float('-inf'), float('inf'))[0]
        # v = float('-inf') if self.index == 0 else float('inf')
        # r = ''
        # print(d)
        # for action in d:
    ...
    #     """*** YOUR CODE HERE ***"""
    #     print(f"In Max_Val Alpha: {alpha} [] Beta: {beta}")
    #     if game_state.is_terminal():
    #         return (game_state.value(), None)
    #     value = alpha
    #     prev_value = value
    #     action = ''
    #     for children in game_state.get_actions():
    #         value = max(value, self.min_val(game_state.generate_successor(children), value, beta)[0])
    #         if prev_value != value:
    #             action = children
    #             prev_value = values
    #         if value >= beta:
    #             print(f"Broke: {value} {beta}")
    #             return (value, action)
    #         elif alpha >= beta:
    #             return (value, action)
    #     return (value, action)
    ...
    #     """*** YOUR CODE HERE ***"""
    #     print(f"In Min_Val Alpha: {alpha} [] Beta: {beta}")
    #     if game_state.is_terminal():
    #         return (game_state.value(), None)
    #     value = beta
    #     prev_value = value
    #     action = ''
    #     for children in game_state.get_actions():
    #         value = min(value, self.max_val(game_state.generate_successor(children), alpha, value)[0])
    #         if prev_value != value:
    #             action = children
    #             prev_value = value
    #         if value <= alpha:
    #             print(f"Broke: {value} {alpha}")
    #             return (value, action)
    #         elif alpha >= beta:
    #             return (value, action)
    #     return (value, action)
